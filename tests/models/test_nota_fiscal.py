...
def test_inserir_nota_fiscal():
    nota_fiscal = NotaFiscal.insertNotaFiscal(valor=valor, numero_serie=numero_serie, descricao=descricao,
...
def test_erro_integridade_mesma_combinacao():
    with pytest.raises(RuntimeError) as exc_info:
        NotaFiscal.insertNotaFiscal(valor=valor, numero_serie=numero_serie, descricao=descricao,
                                    revendedor_fk=revendedor_fk)
...
def test_tipo_dados_incorretos():
    with pytest.raises(TypeError) as exc_info1:
        NotaFiscal.insertNotaFiscal(valor='1', numero_serie=numero_serie, descricao=descricao,
                                    revendedor_fk=revendedor_fk)
    assert 'valor da NotaFiscal deve ser um número!' in str(exc_info1.value)

    with pytest.raises(TypeError) as exc_info2:
        NotaFiscal.insertNotaFiscal(valor=valor, numero_serie=1, descricao=descricao,
                                    revendedor_fk=revendedor_fk)
    assert 'numero_serie da NotaFiscal deve ser uma string!' in str(exc_info2.value)

    with pytest.raises(TypeError) as exc_info3:
        NotaFiscal.insertNotaFiscal(valor=valor, numero_serie=numero_serie, descricao=1,
                                    revendedor_fk=revendedor_fk)
...
def test_valores_vazios():
    with pytest.raises(ValueError) as exc_info2:
        NotaFiscal.insertNotaFiscal(valor=valor, numero_serie='', descricao=descricao,
                                    revendedor_fk=revendedor_fk)
    assert 'numero_serie da NotaFiscal não informado!' in str(exc_info2.value)

    with pytest.raises(ValueError) as exc_info3:
        NotaFiscal.insertNotaFiscal(valor=valor, numero_serie=numero_serie, descricao='',
                                    revendedor_fk=revendedor_fk)
...
def test_fk_nao_existe():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        NotaFiscal.insertNotaFiscal(valor=valor, numero_serie=numero_serie_novo, descricao=descricao,
                                    revendedor_fk=revendedor_fk_fake)
