import pytest
from models.aditivo_nutritivo_picole import AditivoNutritivoPicole, insertAditivoNutritivoPicole
...
def test_inserir_aditivo_nutritivo():
    aditivo_nutritivo = insertAditivoNutritivoPicole(picole_fk=picole_fk, aditivo_nutritivo_fk=aditivo_nutritivo_fk)
...
def test_erro_integridade_mesma_combinacao():
    with pytest.raises(RuntimeError) as exc_info:
        insertAditivoNutritivoPicole(picole_fk=picole_fk, aditivo_nutritivo_fk=aditivo_nutritivo_fk)
...
def test_erro_integridade_chaves_estrangeiras():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        insertAditivoNutritivoPicole(picole_fk=picole_fk_fake, aditivo_nutritivo_fk=aditivo_nutritivo_fk_fake)
...
def test_tipo_dados_incorretos():
    with pytest.raises(TypeError) as exc_info1:
        insertAditivoNutritivoPicole(picole_fk='abc', aditivo_nutritivo_fk=aditivo_nutritivo_fk)

    with pytest.raises(TypeError) as exc_info2:
        insertAditivoNutritivoPicole(picole_fk=picole_fk, aditivo_nutritivo_fk='asdf')
...
def test_inserir_aditivo_nutritivo_combinacao_existente():
    with pytest.raises(RuntimeError) as exc_info:
        insertAditivoNutritivoPicole(picole_fk=1, aditivo_nutritivo_fk=1)
    assert (f"Já existe um AditivoNutritivoPicole com a mesma combinação de picolé e aditivo nutritivo: "
            f"{picole_fk=} | {aditivo_nutritivo_fk=}") in str(exc_info.value)
