...
# @pytest.fixture
# def Lote.insertLote():
#     return Lote.insertLote(picole_fk=picole_fk)
...
# Teste criação de instancias de Lote.insertLote
...
def test_inserir_lote():
    lote = Lote.insertLote(picole_fk=picole_fk, quantidade=quantidade)
...
def test_picole_fkes_vazios():
    with pytest.raises(ValueError) as exc_info2:
        Lote.insertLote(picole_fk=picole_fk, quantidade=0)
...
def test_fk_nao_existe():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        Lote.insertLote(picole_fk=picole_fk_fake, quantidade=quantidade)
