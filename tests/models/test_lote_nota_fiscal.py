...
def test_inserir_lote_nota_fiscal():
    lote_nota_fiscal = LoteNotaFiscal.insertLoteNotaFiscal(nota_fiscal_fk=nota_fiscal_fk, lote_fk=lote_fk)
...
def test_erro_integridade_mesma_combinacao():
    with pytest.raises(RuntimeError) as exc_info:
        LoteNotaFiscal.insertLoteNotaFiscal(nota_fiscal_fk=nota_fiscal_fk, lote_fk=lote_fk)
...
def test_tipo_dados_incorretos():
    with pytest.raises(TypeError) as exc_info1:
        LoteNotaFiscal.insertLoteNotaFiscal(nota_fiscal_fk='1', lote_fk=lote_fk)
    ...
    with pytest.raises(TypeError) as exc_info2:
        LoteNotaFiscal.insertLoteNotaFiscal(nota_fiscal_fk=nota_fiscal_fk, lote_fk='1')
