...
def test_inserir_ingrediente_picole():
    ingrediente_picole = IngredientePicole.insertIngredientePicole(picole_fk=picole_fk, ingrediente_fk=ingrediente_fk)
...
def test_erro_integridade_mesma_combinacao():
    with pytest.raises(RuntimeError) as exc_info:
        IngredientePicole.insertIngredientePicole(picole_fk=picole_fk, ingrediente_fk=ingrediente_fk)
...
def test_erro_integridade_chaves_estrangeiras():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        IngredientePicole.insertIngredientePicole(picole_fk=picole_fk_fake, ingrediente_fk=ingrediente_fk_fake)
...
def test_tipo_dados_incorretos():
    with pytest.raises(TypeError) as exc_info1:
        IngredientePicole.insertIngredientePicole(picole_fk='abc', ingrediente_fk=ingrediente_fk)

    with pytest.raises(TypeError) as exc_info2:
        IngredientePicole.insertIngredientePicole(picole_fk=picole_fk, ingrediente_fk='asdf')
