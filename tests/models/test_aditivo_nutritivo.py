...
from models.aditivo_nutritivo import insertAditivoNutritivo
...
def test_inserir_aditivo_nutritivo():
    aditivo_nutritivo = insertAditivoNutritivo(nome='VITAMIna D', formula_quimica='C27H44O')
    ...
    aditivo_nutritivo = insertAditivoNutritivo(nome='VITAMIna E', formula_quimica='C29H50O2')
    assert (aditivo_nutritivo.id is not None and
...
def test_erro_integridade_mesma_combinacao():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        insertAditivoNutritivo(nome=nome, formula_quimica=formula_quimica)
...
def test_tipo_dados_incorretos():
    with pytest.raises(TypeError) as exc_info1:
        insertAditivoNutritivo(nome=1, formula_quimica='C27H44O')

    with pytest.raises(TypeError) as exc_info2:
        insertAditivoNutritivo(nome='VITAMIna D', formula_quimica=1)
...
def test_valores_vazios():
    with pytest.raises(ValueError) as exc_info1:
        insertAditivoNutritivo(nome='', formula_quimica='C27H44O')

    with pytest.raises(ValueError) as exc_info2:
        insertAditivoNutritivo(nome='VITAMIna D', formula_quimica='')
