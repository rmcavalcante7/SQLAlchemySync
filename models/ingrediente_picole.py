...
def selectAllIngPicPorPicoleFK(picole_fk: int) -> list or []:
    ...
    try:
        ...
        with createSession() as session:
            ingredientes_picole = session.query(IngredientePicole).filter(IngredientePicole.picole_fk == picole_fk).all()
            return ingredientes_picole
...
def selectAllIngPicPorIngredienteFK(ingrediente_fk: int) -> list or []:
    ...
    try:
        ...
        with createSession() as session:
            ingredientes_picole = session.query(IngredientePicole).filter(IngredientePicole.ingrediente_fk == ingrediente_fk).all()
            return ingredientes_picole
...
if __name__ == '__main__':
    try:
        insertIngredientePicole(picole_fk=1, ingrediente_fk=1)
    ...
    try:
        insertIngredientePicole(picole_fk=1, ingrediente_fk=2)
    except Exception as e:
        print(f'Erro ao inserir IngredientePicole: {e}')

    try:
        insertIngredientePicole(picole_fk=100, ingrediente_fk=100)
    except Exception as e:
        print(f'Erro ao inserir IngredientePicole: {e}')
